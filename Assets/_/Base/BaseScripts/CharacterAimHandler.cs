...
public class CharacterAimHandler : MonoBehaviour, EnemyHandler.IEnemyTargetable
{
    ...
    private void Start()
    {
        ...
        V_UnitSkeleton_Composite_Weapon unitSkeletonCompositeWeapon = new V_UnitSkeleton_Composite_Weapon(vObject,
            unitSkeleton, GameAssets.UnitAnimEnum.dMarine_AimWeaponRight,
            GameAssets.UnitAnimEnum.dMarine_AimWeaponRightInvertV, GameAssets.UnitAnimEnum.dMarine_ShootWeaponRight,
            GameAssets.UnitAnimEnum.dMarine_ShootWeaponRightInvertV);
        vObject.AddRelatedObject(unitSkeletonCompositeWeapon);
        unitSkeletonCompositeWeapon.SetActive();

        V_UnitSkeleton_Composite_Walker unitSkeletonCompositeWalker_BodyHead =
            new V_UnitSkeleton_Composite_Walker(vObject, unitSkeleton, GameAssets.UnitAnimTypeEnum.dMarine_Walk,
                GameAssets.UnitAnimTypeEnum.dMarine_Idle, new[] {"Body", "Head"});
        vObject.AddRelatedObject(unitSkeletonCompositeWalker_BodyHead);

        V_UnitSkeleton_Composite_Walker unitSkeletonCompositeWalker_Feet = new V_UnitSkeleton_Composite_Walker(vObject,
            unitSkeleton, GameAssets.UnitAnimTypeEnum.dMarine_Walk, GameAssets.UnitAnimTypeEnum.dMarine_Idle,
            new[] {"FootL", "FootR"});
        ...
        FunctionUpdater.Create(() =>
        {
            ...
            RaycastHit2D raycastHit = Physics2D.Raycast(gunEndPointPosition,
            ...
            if (canShoot && Input.GetMouseButton(0))
            {
                ...
                unitSkeleton.ReplaceBodyPartSkeletonAnim(GameAssets.UnitAnimTypeEnum.dMarine_Attack.GetUnitAnim(aimDir),
                    "Body", "Head");
                // Shoot Composite Skeleton
                unitSkeletonCompositeWeapon.Shoot(targetPosition, () => { canShoot = true; });
            ...
            raycastHit = Physics2D.Raycast(GetPosition() + moveDir * .1f, moveDir,
    ...
    private static V_Object CreateBasicUnit(Transform unitTransform, Vector3 spawnPosition, float walkerSpeed,
        Material materialSpriteSheet)
    {
        ...
        V_IObjectTransform transform = new V_ObjectTransform_LateUpdate(instantiatedTransform,
            () => instantiatedTransform.transform.position, V_Main.RegisterOnLateUpdate, V_Main.DeregisterOnLateUpdate);
        unitObject.AddRelatedObject(transform);
        V_IObjectTransformBody transformBody =
            new V_ObjectTransformBody(instantiatedTransform.Find("Body"), materialSpriteSheet);
