...
namespace GridPathfindingSystem
{
    public class UnitMovement
    {
        ...
        public void OnPathComplete(List<PathNode> path, MapPos _mapPos)
        {
            ...
            */
            ...
            if (path.Count > 1 && (previousFinalPos == null ||
                                   (previousFinalPos.offsetX == 0 && previousFinalPos.offsetY == 0)))
            {
                if (Vector3.Distance(getPosition(), new Vector3(path[1].xPos * 10, path[1].yPos * 10)) <
                    Vector3.Distance(new Vector3(path[0].xPos * 10, path[0].yPos * 10),
        ...
        public bool MoveTo(MapPos _mapPos,
            GridPathfinding.UnitMovementCallbackType _callbackType = GridPathfinding.UnitMovementCallbackType.Simple,
        ...
        public bool MoveTo(List<MapPos> _mapPos, GridPathfinding.UnitMovementCallbackType _callbackType, object _obj,
            PathCallback callback)
        {
            lastMoveTo = new LastMoveTo(_mapPos, _callbackType, _obj, callback);
            callbackType = _callbackType;
