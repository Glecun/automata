...
public class Generic_Mesh_Script : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            presetUV[i * 4 + 0] = new Vector2(UVList[i].Vector_1.x / UVTextureSize.x,
                UVList[i].Vector_1.y / UVTextureSize.y);
            presetUV[i * 4 + 1] = new Vector2(UVList[i].Vector_2.x / UVTextureSize.x,
                UVList[i].Vector_2.y / UVTextureSize.y);
            presetUV[i * 4 + 2] = new Vector2(UVList[i].Vector_3.x / UVTextureSize.x,
                UVList[i].Vector_3.y / UVTextureSize.y);
            presetUV[i * 4 + 3] = new Vector2(UVList[i].Vector_4.x / UVTextureSize.x,
                UVList[i].Vector_4.y / UVTextureSize.y);
        }
    ...
    private void addGeneric(Vector3 pos, float rot, int material, float offset, Vector3 baseDir, bool skewed)
    {
        ...
        if (index == 0 && spawnNewMeshWhenFull)
        {
            ...
            GameObject newInstance = new GameObject(name, typeof(MeshFilter), typeof(MeshRenderer),
                typeof(Generic_Mesh_Script));
    ...
    public void updateGeneric(int index, Vector3 pos, float rot, int material, float offset, Vector3 baseDir,
        bool skewed)
    {
        update = true;
        //pos.z -= offset;
    ...
    public static void AddGeneric(string meshName, Vector3 pos, float rot, int material, float offset, Vector3 baseDir,
        bool skewed)
    {
        dictionary[meshName].addGeneric(pos, rot, material, offset, baseDir, skewed);
    }

    public static void UpdateGeneric(string meshName, int index, Vector3 pos, float rot, int material, float offset,
        Vector3 baseDir, bool skewed)
    {
