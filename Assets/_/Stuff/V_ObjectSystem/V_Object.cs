    ...
    public class V_Object
    {
        ...
        */
        ...
        public static V_Object CreateObjectPrefab(GameObject prefab, Vector3 position)
        {
            ...
            Transform instancedTransform =
                UnityEngine.Object.Instantiate(prefab, position, Quaternion.identity).transform;
        ...
        public static V_Object CreateSkeleton(Vector3 position, Material material)
        {
            ...
            V_UnitSkeleton unitSkeleton = new V_UnitSkeleton(1f, transformBody.ConvertBodyLocalPositionToWorldPosition,
            ...
            Vector3 dir, float frameRateMod)
        {
            ...
            instancedObject.GetLogic<V_UnitAnimation>()
                .PlayAnimForced(unitAnimType, dir, frameRateMod, onAnimComplete, null, null);
            return instancedObject;
        }
        ...
        public static V_Object CreateSkeletonPresetAnim(Vector3 position, Material material, UnitAnimType unitAnimType,
            Vector3 dir, float frameRateMod, Vector3 rotatePrefabDir, Vector3 scalePrefab)
        {
            V_Object instancedObject = CreateSkeleton(position, material);
            V_UnitSkeleton.OnAnimComplete onAnimComplete = delegate(UnitAnim unitAnim)
            {
        ...
        public static V_Object CreateSkeletonPresetAnim(Vector3 position, Material material, UnitAnim unitAnim,
            float frameRateMod, Vector3 rotatePrefabDir, Vector3 scalePrefab, int sortingOrderOffset)
        {
            V_Object instancedObject = CreateSkeleton(position, material);
            V_UnitSkeleton.OnAnimComplete onAnimComplete = delegate(UnitAnim u) { instancedObject.DestroySelf(); };
            instancedObject.GetLogic<V_IObjectTransform>().GetTransform()
                .GetComponent<CodeMonkey.MonoBehaviours.PositionRendererSorter>().SetOffset(sortingOrderOffset);
            instancedObject.GetLogic<V_UnitAnimation>()
                .PlayAnimForced(unitAnim, frameRateMod, onAnimComplete, null, null);
            instancedObject.GetLogic<V_IObjectTransform>().SetEulerZ(GetAngleFromVector(rotatePrefabDir));
            instancedObject.GetLogic<V_IObjectTransform>().SetScale(scalePrefab);
            return instancedObject;
        }

        public static V_Object CreateSkeletonPresetAnim_DebugDash(Vector3 position, Vector3 rotationDir,
            Vector3 localScale)
        {
            V_Object vObject = V_Object.CreateSkeletonPresetAnim(position, GetWhitePixelMaterial(),
                UnitAnim.GetUnitAnim("EffectDash"), 1f, rotationDir, localScale, 100);
            return vObject;
        }
        ...
        {
            V_Object vObject = V_Object.CreateSkeletonPresetAnim(position, GetWhitePixelMaterial(),
                UnitAnim.GetUnitAnim("EffectDash"), 1f, new Vector3(0, 1, 0), new Vector3(3f, 1f, 1f), 100);
            return vObject;
        }
        ...
        public static V_Object CreateSkeletonPresetAnim_DebugDash(Vector3 position, Color color, float frameRate = 1f)
        {
            ...
            V_Object vObject = V_Object.CreateSkeletonPresetAnim(position, material, UnitAnim.GetUnitAnim("EffectDash"),
                frameRate, new Vector3(0, 1, 0), new Vector3(3f, 1f, 1f), 100);
            return vObject;
        }
        ...
        public static List<V_Object> FilterObjectList(List<V_Object> objectList,
        ...
        public static List<V_Object> FilterObjectList_Distance(List<V_Object> objectList, Vector3 position,
            float distance)
        {
        ...
        public static V_Object GetClosestFromList(Vector3 position, List<V_Object> objectList)
        {
            ...
            for (int i = 0; i < objectList.Count; i++)
            {
                ...
                {
                    // No closest
                    if (Vector3.Distance(position, objectList[i].GetPosition()) <
        ...
        {
            return delegate(V_Object vObject) { return Vector3.Distance(position, vObject.GetPosition()) <= distance; };
        }
