...
namespace V_AnimationSystem
{
    public static class V_Animation
    {
        ...
        private static void Init_Folders()
        {
            ...
            if (!System.IO.Directory.Exists(LOC_ANIMATIONTYPES))
                System.IO.Directory.CreateDirectory(LOC_ANIMATIONTYPES);
            if (!System.IO.Directory.Exists(LOC_DEFAULT_ANIMATIONS))
                System.IO.Directory.CreateDirectory(LOC_DEFAULT_ANIMATIONS);
            if (!System.IO.Directory.Exists(LOC_DEFAULT_ANIMATIONTYPES))
                System.IO.Directory.CreateDirectory(LOC_DEFAULT_ANIMATIONTYPES);
        }
        ...
        public static V_Skeleton_Anim[] V_LoadAnimationString(string readAllText, int animationFrameMultiplier = 2)
        {
            ...
            animKeyframes =
                V_Animation.Load_List<V_Skeleton_Anim>(content[0], V_Skeleton_Anim.Load, "#SKELETONANIMLIST#");
