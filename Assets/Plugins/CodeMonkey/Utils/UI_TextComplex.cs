...
namespace CodeMonkey.Utils
{
    ...
    public class UI_TextComplex
    {
        ...
        public UI_TextComplex(Transform parent, Vector2 anchoredPosition, int fontSize, char iconChar, string text,
            Icon[] iconArr, Font font)
        {
            ...
            while (tmp.IndexOf(iconChar) != -1)
            {
                ...
                if (untilTmp.Trim() != "")
                {
                    Text uiText = UtilsClass.DrawTextUI(untilTmp, transform, new Vector2(textPosition, 0), fontSize,
                        font);
                ...
                UtilsClass.DrawSprite(icon.sprite, transform, new Vector2(textPosition + icon.size.x / 2f, 0),
                    icon.size);
