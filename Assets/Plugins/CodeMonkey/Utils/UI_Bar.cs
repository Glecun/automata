...
namespace CodeMonkey.Utils
{
    ...
    public class UI_Bar
    {
        ...
        public UI_Bar(Transform parent, Vector2 anchoredPosition, Vector2 size, Color backgroundColor, Color barColor,
            float sizeRatio)
        {
            SetupParent(parent, anchoredPosition, size);
        ...
        public UI_Bar(Transform parent, Vector2 anchoredPosition, Vector2 size, Color backgroundColor, Color barColor,
            float sizeRatio, Outline outline)
        {
            SetupParent(parent, anchoredPosition, size);
            if (outline != null) SetupOutline(outline, size);
            SetupBackground(backgroundColor);
        ...
        {
            return UtilsClass.DrawSprite(outline.color, gameObject.transform, Vector2.zero,
                size + new Vector2(outline.size, outline.size), "Outline");
        }

        private void SetupBackground(Color backgroundColor)
        {
            background = UtilsClass.DrawSprite(backgroundColor, gameObject.transform, Vector2.zero, Vector2.zero,
                "Background");
